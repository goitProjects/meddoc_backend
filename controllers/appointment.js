...
module.exports.updateAppointment = async (req, res) => {
  const patient = ObjectId(
    req.user.role === "patient" ? req.user._id : req.body.opponent
  );
  const doctor = ObjectId(
    req.user.role === "doctor" ? req.user._id : req.body.opponent
  );
  ...
  if (app) {
    return res
      .status(200)
      .json({ acknowledged: false, date: "busy", item: app._id });
  }
...
module.exports.addAppointment = async (req, res) => {
  ...
  const app = await Appointment.findOne(query);
  if (!app) {
    const newApp = await Appointment.collection.insertOne({
      patient,
      doctor,
